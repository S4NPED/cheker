...
echo ""
echo "=== ПРОВЕРКА ЗАДАНИЙ НА HQ-SRV ==="
echo ""
...
if [ "$YEAR" == "2025" ]; then
    ...
    echo "Задание 1: Проверка имени хоста и сетевого интерфейса"
    echo "Проверка имени хоста:"
    if [ "$(hostname)" == "hq-srv.au-team.irpo" ]; then
        echo "✓ Имя хоста корректно: $(hostname)"
    else
        echo "✗ Ошибка: имя хоста должно быть 'hq-srv.au-team.irpo', сейчас: $(hostname)"
    fi

    echo ""
    echo "Проверка сетевого интерфейса"
    if ip -4 addr show ens3 | grep -q "192.168.100.2/26"; then
        echo "✓ IP-адрес корректный: 192.168.100.2/26"
    else
    ...
    echo ""
    echo "Задание 10: Проверка DNS (BIND9)"
    if systemctl is-active bind9 > /dev/null; then
        ...
        if named-checkconf -z &>/dev/null; then
            ...
            # Проверка зон
            echo "  Проверка зон:"
            named-checkconf -z 2>&1 | grep -E "zone.*loaded"
            
            # Проверка работы DNS
            if dig @localhost hq-srv.au-team.irpo +short | grep -q "192.168.100.2"; then
                echo "✓ Прямое преобразование работает"
            else
                echo "✗ Ошибка: прямое преобразование не работает"
            fi
        else
            echo "✗ Ошибка: синтаксическая ошибка в конфигурации BIND9"
            named-checkconf -z 2>&1 | head -10
        fi
    else
        echo "✗ Ошибка: BIND9 не запущен"
    fi
    ...
    echo ""
    echo "Проверка сетевого интерфейса"
    if ip -4 addr show ens3 | grep -q "192.168.100.2/27"; then
        echo "✓ IP-адрес корректный: 192.168.100.2/27"
    else
        echo "✗ Ошибка: неправильный IP-адрес на ens3"
    fi
